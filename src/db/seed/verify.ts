...
async function verifyDatabase(): Promise<void> {
  ...
  try {
    ...
    if (orphanedUnits === 0 && orphanedChunks === 0 && orphanedResources === 0 && orphanedExercises === 0) {
      console.log('[SUCCESS] All relationships are intact - no orphaned records found');
    } else {
      console.log(`[ERROR] Found issues: ${orphanedUnits} orphaned units, ${orphanedChunks} orphaned chunks, ${orphanedResources} orphaned resources, ${orphanedExercises} orphaned exercises`);
    }
    ...
    if (allCourses.length > 0) {
      ...
      if (courseUnits.length > 0) {
        ...
        if (unitChunks.length > 0) {
          ...
          console.log(`    Chunk: ${chunk.title} (order: ${chunk.order}, time: ${chunk.timeMinutes}min)`);
          ...
          console.log(`    Resources: ${chunkResources.length}, Exercises: ${chunkExercises.length}`);
