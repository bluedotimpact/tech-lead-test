...
class DatabaseSeeder {
  ...
  private async loadCsvData() {
    ...
    console.log(`[INFO] Loaded: ${courses.length} courses, ${units.length} units, ${chunks.length} chunks, ${exercises.length} exercises, ${chunkResources.length} chunk-resources`);
  ...
  private async seedChunks(chunksData: ChunkCSV[], unitsData: UnitCSV[]): Promise<void> {
    ...
    for (const csvChunk of chunksData) {
      try {
        ...
        if (!matchingUnit) {
          throw new Error(`Unit not found for chunk: ${csvChunk.Title} (looking for unit: ${unitName})`);
        }
  ...
  private async seedResources(resourcesData: ChunkResourceCSV[]): Promise<void> {
    ...
    for (const csvResource of resourcesData) {
      try {
        ...
        if (!chunkId) {
          throw new Error(`Chunk not found for resource: ${csvResource['[>] Resource name']} (looking for chunk: ${chunkName})`);
        }
    ...
    for (const csvExercise of exercisesData) {
      try {
        ...
        if (!chunkId) {
          ...
          console.log(`[INFO] Skipping exercise (no chunk found): ${csvExercise.Title} (chunk: ${chunkName})`);
          continue;
        }
