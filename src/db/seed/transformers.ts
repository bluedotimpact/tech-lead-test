...
export function transformCourse(csvData: CourseCSV): NewCourse {
  ...
  const courseName =
    cleanText(
      csvData.Course || ((csvData as unknown as Record<string, unknown>)["﻿Course"] as string)
    ) || "Untitled Course";
...
export function transformResource(csvData: ChunkResourceCSV, chunkId: string): NewResource {
  ...
  const validTypes: ("Article" | "Blog" | "Paper" | "Website")[] = [
    "Article",
    "Blog",
    "Paper",
    "Website",
  ...
  const validStatuses: ("Core" | "Maybe" | "Supplementary" | "Optional")[] = [
    "Core",
    "Maybe",
    "Supplementary",
    "Optional",
 ...
 */
export function findChunkForResource(
  resourceData: ChunkResourceCSV,
  chunkNameMap: Map<string, ChunkCSV>
): ChunkCSV | null {
 ...
 */
export function findChunkForExercise(
  exerciseData: ExerciseCSV,
  chunkNameMap: Map<string, ChunkCSV>
): ChunkCSV | null {
